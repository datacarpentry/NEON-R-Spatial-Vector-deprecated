...
We will first, importing three shapefiles. The first is our `AOI` or area of interest
...
the location of roads within the field site. Finally we will inport a file containing
...
Shapefile metadata, as imported into an `R` spatial object can be accessed using 
the `class()`, feature count (`length()`), object `extent()`, and object 
coordinate reference system (`CRS`) methods. Let's explore the metadata for our
`point_HARV` object. 
...
1. Determine the `extent`, `CRS`, number of objects and `class` for the `lines_HARV` and 
`aoiBoundary_HARV` objects.
2. Is the `CRS` the same for `lines_HARV`, `aoiBoundary_HARV`, & `point_HARV`?
3. What are the NAMES of the attributes associated with the `point_HARV` object?
</div>
...
#just view the attributes & attribute values of the data
lines_HARV@data
...
#view just the first 6 attribute values for each attribute in the the lines 
#spatial object
head(lines_HARV@data)
...
it will be much easier to plot! Let's give it a go!
...
Great - We now have a plot of roads at Harvard Forest by `TYPE` which is an `R`
`factor. We can add a legend to our plot too. When we add a legend, we can use
...
* we can use `fill=palette()` to color the boxes in our legend. `palette()` is the 
