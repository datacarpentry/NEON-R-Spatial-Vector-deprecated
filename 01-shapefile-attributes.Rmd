...
description: "This lesson explains the nature of shapefile attributes. Participants will be able to locate and query shapefile attributes as well as subset shapefiles by specific attribute values."
code1: 
image:
...
2. Who owns the site in the `point` data object?
...
We can also explore individual values stored within a particular attribute. Again,
...
#base plot - you may already have this done
plot(aoiBoundary, col="cyan2", main="Harvard Forest Tower")  #map Boundary polygon
plot(point, col="red", add=TRUE)   #add tower point 
plot(lines, add=TRUE)  #add in roads  
