...
is an individual **attribute** that describes an object (sometimes called a
variable). Shapefile attributes include measurements that correspond to the
...
which describes the type of road (woods road, footpath, boardwalk, or stonewall).
...
Notice that two of our attributes have space in them: stone wall and woods road.
There are really four TYPEs, not six TYPEs.  
...
We can also plot our subsetted shapefiles.
...
To plot vector data with the color determined by a set of attribute values,
we must convert an attribute that is `categorical` data to class=
`factor`. A **factor** is similar to a category - you can group vector objects
by a particular category value - for example you can group all lines of
`TYPE=footpath`. However, in `R`, a factor can also have a determined *order*. 
