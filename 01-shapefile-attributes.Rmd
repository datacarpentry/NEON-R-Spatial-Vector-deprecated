...
When we type in the full object name, we see the same three types of metadata
with some additional attribute information.  
...
`levels` is particularly useful if we have an object with lots of different
lines, polygons or points as it summarizes the differences for us.  
...
# That looks pretty good.  Let's save it as a base plot for future use. When 
# using the base `R` plot() function we have to use recordPlot() to create an
# object. 
Harv_baseMap<- recordPlot()
