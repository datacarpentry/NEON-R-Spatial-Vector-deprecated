...
More about the NEON Harvard Forest field site</a>. These data are a proxy of the 30 year time-series that will be collected from the NEON flux tower and made available
...
We can query the layer type either by entering the name of the `R` object into the console or by calling the `class()` function. In this case, we see that our object is a type of polygon layer called a `SpatialPolygonsDataFrame`.
