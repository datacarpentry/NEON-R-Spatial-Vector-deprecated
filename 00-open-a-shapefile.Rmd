...
description: "This lesson explains the how to open and plot point, line, and polygon shapefiles in R."
code1: 
image:
...
<a href="http://www.neoninc.org/science-design/field-sites/harvard-forest" target="_blank"> More about the NEON Harvard Forest field site</a>.
...
More about the NEON Harvard Forest field site</a>. These data are a proxy of the 30 year 
time-series that will be collected from the NEON flux tower and made available
<a href="http://data.neoninc.org" target=_"blank">from the NEON data portal</a>.
{: .notice}
 
###Notes about R Libraries

We will use the `rgdal` library (package) to work with vector data in R. Notice that `sp`
is automatically loaded when `rgdal` is loaded. We will use the `raster`
...
We can query the layer type either by entering the name of the `R` object into the console 
or by calling the `class()` function. In this case, we see that our object is a type of 
polygon layer called a `SpatialPolygonsDataFrame`.

Bonus:  The acronym, `OGR` refers to the OpenGIS Simple Features Reference 
Implementation. 
<a href="https://trac.osgeo.org/gdal/wiki/FAQGeneral" target="_blank"> Learn more about OGR.</a>
{ : .notice }  

The first shapefile that we will open, contains the boundary of our study area
(or our Area Of Interest, hence the name `aoiBoundary`). 
...
NOTE: we can also use the `attributes()` method to view objects attributes
{: .notice }
...
>#Challenge: Import a line and point shapefile
>
...
classifications of the object. Attribute data can be any sort of information that you might 
want to associate with your spatial objects. Each spatial 
