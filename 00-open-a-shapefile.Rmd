...
Each vector in the shapefile often has a set of associated `attributes` that describe the data.
...
Whenever adding a new file it is good coding practice to include what the file
is and it source.  [In the initial what is the additional metadata added and
why? ->add to description ]
...
giving the extent of the file.  We will discuss CRS and extent a little later.
Finally we see there is one variable named "id" that appears to consists of only ones (min and max value both 1).
...
Now that we see just the attributes we see that the variable “id” actually consists of 0s and 1s.  [Why the difference? R doesn’t recognize 0 as data but as null?]
...
While we previoulsy viewed this data by just entering the object name
`aoiBoundary` we can view the just CRS or Extent for the shapefile using the 
following commands:
...
Each spatial object in a shapefile can have the same set of attributes. These
attributes can be any sort of attribute that you might store in a spreadsheet 
about your data. 
