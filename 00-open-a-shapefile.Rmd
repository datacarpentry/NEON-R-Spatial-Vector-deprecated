...
<figure>
    ...
    <figcaption> Vector data can be a point, line or polygon. 
    Image Source: NEON,Inc.  
    </figcaption>
</figure>

* **Points:** Each individual point is represented by a single x,y coordinate.
There can be many points in a vector point file. Examples: sampling
...
not an issue we will deal with further.
 ...
 <i class="fa fa-star"></i> **Data Tip:** a line can represent a boundary as well - however, a line will not
create a closed object.  
{: .notice}
...
To work with vector data in `R`, we can use the `rgdal` library. Notice that the
`sp` package is automatically loaded when `rgdal` is loaded. The `raster` 
...
information about and associated with the data. We are particularly interested
in the geospatial *metadata* that describes the format, `CRS`, `extent`, and
other components of the dataset, as well as, 
*attributes* which contains properties associated with each individual item
stored in the vector dataset.
...
in the CRS UTM zone 18N. This information is critical to interpreting the
object `extent`. 
...
lines_HARV <- readOGR("NEON-DS-Site-Layout-Files-HARV/HARV",layer = "HARV_roads")
#import point shapefile
point_HARV <- readOGR("NEON-DS-Site-Layout-Files-HARV/HARV", layer="HARVtower_UTM18N")
...
For more on parameter options in base `R` `plot()`, check out these
resources:
