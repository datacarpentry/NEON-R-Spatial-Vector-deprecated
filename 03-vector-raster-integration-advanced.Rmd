...
the extent of a vector shapefile. The skills to be able to extract
values from a raster that correspond to the geometry of a vector overlay are
also taught."
code1: 
image:
...
plot(aoiBoundary, main = "Cropped CHM Raster")
...
```{r challenge-code-crop-raster-points, echo=FALSE}
...
* More on the 
[extent class in R](http://www.inside-r.org/packages/cran/raster/docs/extent)
...
```{r challenge-code-extract-plot-tree-height, Echo=FALSE}
