...
bring along metadata and attriubute data. 
...
points in the header we just looked at). Luckily this is similar to our boundary
shapefiles that we opened previously. 
...
#
plot.locationSp <- SpatialPointsDataFrame(plot.location[,1:2],    #x,y coordinates
                    plot.location,                    #?
                    proj4string = crs(aoiBoundary))    # assinging the CRS system
                                                      # in our shapefile
...
What happenned to the tower?  The extent of these data are much larger than the
bouldery file that we originally originally projected.  
...
Often, we have data that are in different CRSs. In order to map these objects
in the same visual space, we need to reproject one or the other. We can use the 
