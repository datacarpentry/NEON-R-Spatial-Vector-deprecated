...
description: "This lesson walks through how to convert a .csv file taht contains
...
reproject data in a shapefile format into another projection, create a 
...
The vector data used in this tutorial were collected  at the
...
Converting from .csv to Shapefile in R</a>
...
NEON <a href="http://www.neoninc.org/science-design/collection-methods/terrestrial-organismal-sampling" target="_blank"> 
field crews are sampling vegetation and other 
ecological metrics. We would like to
...
both the x,y values that are the coordinates but also the columns describing associated metadata or associated attributes.  

Note: There is a `SpatialPoints` object (not `SpatialPointsDataFrame`) in `R`
that does not allow you to store associated attributes. {: .notice1}
...
#Read the .csv file
plot.locations_HARV <- read.csv("boundaryFiles/HARV/HARV_PlotLocations.csv")
...
``` {r  challenge-code-crs, echo=FALSE}
...
Okay, there are several colums related to a CRS.  Let's check out the first to
...
* +zone=18      the UTM zone -- we know our data is in 18N but N isnt' needed
...
* +towgs84=0,0,0  conversion from current datum/ellpse to WGS84 -- nothing,
we're already in WGS84 for both.  

* For more information on [proj4](http://proj.maptools.org/faq.html).
* For information on datums in `R`: `projInfo(type = "datum")`

``` {r convert-csv-shapefile}
#Which data.frame columns represent the UTMs?
names(plot.locations_HARV)
#X, Y are 1:2 or easting,northing are 23:24.  We could use either
...
plot.locationSp_HARV <- SpatialPointsDataFrame(plot.locations_HARV[,23:23],
                # [,23:24]the columns which represent UTM coordinate locations 
                    plot.location_HARV,    #The R object to convert
                    proj4string = CRS_plot_HARV)   # assign a CRS 
                                          
#look at CRS
crs(plot.locationSp_HARV)
...
```{r plot-data-points }
plot(plot.locationSp_HARV, main="Map of Plot Locations")
...
plot.location_HARV_matchCRS <- SpatialPointsDataFrame(plot.location_HARV[,23:24],
                                plot.location,    # object to convert
                                proj4string = crs(aoiBoundary))   #assign CRS to
                                                    #match `aoiBoundary_HARV CRS
crs(plot.location_HARV_matchCRS)
```
...
* the name of the spatial object (`plot.locationSp_HARV`)
...
``` {r write-shapefile}
#write a shapefile
writeOGR(plot.locationSp_HARV, getwd(),
...
newPlots_HARV <- readOGR("boundaryFiles/HARV", "newPlots_latLon")
...
Now that the `CRS`s match we can add our new plot points to our existing map.  
...
Yes!  A nice blue points showed up for our new plot... but wait only one.  We 
...
plot(plot.locationSp_HARV, add=T)
...
We will use a Canopy Heigt Model (`chm`) from the Harvard forest as the base
...
Using a Digital Terrain Model ("NEON_RemoteSensing/HARV/DTM/HARV_dtmCrop.tif") 
create a usable map to the study area.  
...
```{r challenge-code-plotting-pretty-map, echo=FALSE}
